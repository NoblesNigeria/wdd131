...
const TempleDetails= displayTempleCards();
filteredTempleCards(templeDetails);
...
oldTempleNav.addEventListener('click', () => {
  const filteredTemples = templeDetails.filter(temple => {
    return parseInt(temple.dedicated) < 1900;
  });

  filteredTempleCards(filteredTemples);
});


function displayTempleCards(templesList) {
  const container = document.querySelector(".temple-cards");
